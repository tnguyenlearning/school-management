...
const StudentAccountTable = ({ data, loading }) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    <Button onClick={() => navigate(`${urls.baseBalanceDetails}/${record.id}`)}>Details</Button>
                    <Button onClick={() => navigate(`${urls.baseReceiptForm}/create/${record.studentCode}/${record.id}`)}>Create Receipt</Button>
                </Space>
            ),
        },
    ];
